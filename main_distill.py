...
from utils.pred_utils import AverageMeter, set_seed, save_checkpoint
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    checkpoint = torch.load("pretrainmodel/deit_base_patch16_224-b5f2ef4d.pth", map_location=loc)
    ...
    print("=> loaded teacher checkpoint '{}'" .format(args.resume))
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        batch_size=int(args.batch_size * 8), shuffle=False,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_acc1, train_acc5, train_loss = train(train_loader, model, teacher_model, criterion, optimizer, epoch, scaler, args)
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed  and args.rank % ngpus_per_node == 0):
        ...
        with open(logname, 'a') as logfile:
            ...
            logwriter.writerow([epoch, train_loss, train_acc1.item(), train_acc5.item(),validate_loss, validate_acc1.item(), validate_acc5.item() ])
    folder_path = 'checkpoint/'
    file_name = folder_path + args.name + "_ema_model.pth"
    save_checkpoint(model.state_dict(), False, filename=file_name)

def train(train_loader, model, teacher_model, criterion, optimizer, epoch, scaler, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # input, targets_a, targets_b, lam = mixup_data(images, target, args.alpha, True)
        with autocast():
